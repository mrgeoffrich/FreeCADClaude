...
cd gcode_web && npx vitest run && cd ..      # the G-code viewer's parser guard
...
freecadcmd /abs/path/to/eval/test_gcode_server.py    # the loopback viewer server
freecadcmd /abs/path/to/eval/test_slicer_runner.py   # presets, argv, the job table
freecadcmd /abs/path/to/eval/test_slice_tools.py     # the slice tools' error paths
freecadcmd /abs/path/to/eval/test_export_3mf.py      # 3MF keeps objects separate
freecadcmd /abs/path/to/eval/test_oriented_export.py # parts stood up the way they print
```
...
is what anyone reviewing the change can actually read.
